...
def plot_tree(decision_tree, target, counter, multigraph):
    # checking if the tree has a leaf as root
    if not hasattr(decision_tree, 'branch'):
        root_id = f"{counter}_root"
        multigraph.add_node(root_id, label = decision_tree) # because it is a leaf
        counter += 1
        return root_id, counter
    else:
        plot_tree_recursive(decision_tree, target, counter, multigraph)

# defining plotting tree function
def plot_tree_recursive(decision_tree, target, counter, multigraph): # where target is the list of the possible dataset's targets
    ...
    for i in range(len(decision_tree.branch.subtree)):
        ...
        # recursive call for the subtrees
            ...
            node_id, counter = plot_tree_recursive(decision_tree.branch.subtree[i], target, counter, multigraph) # keeping track of counter in order to have separate nodes
